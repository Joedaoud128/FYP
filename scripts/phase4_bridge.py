...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Phase 4 bridge: run self-correction on generated code.")
    parser.add_argument("generated_file", help="Path to Python file from an upstream microservice.")
...
def main() -> int:
    ...
    if args.check_runtime and not service.is_runtime_healthy():
        print(json.dumps({"mode": "phase4-bridge", "runtime_healthy": False, "message": "Local Ollama runtime is unavailable."}, indent=2))
        return 2

    result = service.run_target_file(args.generated_file)
